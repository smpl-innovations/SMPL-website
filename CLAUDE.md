...
- **`styles.css`** — single stylesheet, no preprocessor. The design system lives in `:root` CSS custom properties: a strict navy palette (`--navy-deep #0b1628`, `--navy-mid`, `--blue-accent #2563eb`, `--cyan-accent`), a white tone ladder (`--white-soft/muted/dim/faint/ghost/bright`), `--accent-border`, radii (`--radius-lg/md/sm/pill`), shadows (`--surface-shadow`, `--surface-shadow-sm`), a spacing scale (`--space-2xs…3xl` = 8/12/18/24/28/36/44/64px + `--space-section`), type-size ramps (`--fs-title-*`, `--fs-body-*`, `--fs-mono-*`), `--max` container width, and three type tokens — `--font-display` (Archivo, used expanded via `font-stretch`), `--font-body` (Inter), `--font-mono` (Fragment Mono) — all self-hosted woff2 in `assets/fonts/`. **Use tokens, not literals**: spacing tokens for gaps/card paddings, and palette alphas written as `color-mix(in srgb, var(--token) N%, transparent)` — no raw `rgba()` outside `:root`. The canvas colors in `script.js` intentionally mirror `--blue-glow` as literals; change them in tandem with the palette. The blueprint surface/motion system (hairline section dividers, dot grids, grain overlay, CSS-counter section numbering, scroll-driven reveals, cursor spotlight) and the full token list are documented in `Design.md` under "Design System v2".
